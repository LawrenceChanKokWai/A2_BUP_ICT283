...
void TestDateNotEqualsOperator( const Date &thisDate, const Date &otherDate );
void TestDateLessThanOperator( const Date &thisDate, const Date &otherDate );
void TestDateGreaterThanOperator( const Date &thisDate, const Date &otherDate );
...
int main()
{
    ...
    TestDateNotEqualsOperator( dateValid, anotherDate );
    TestDateLessThanOperator( anotherDate, dateValid );
    TestDateGreaterThanOperator( dateValid, anotherDate );
...
void TestDateEqualsOperator( const Date &thisDate, const Date &otherDate )
{
    Assert( thisDate == otherDate,
            ...
            "Test Data/Values: thisDate, otherDate\n",
...
{
    Assert( thisDate != otherDate,
            "Test 8: operator != Overloading - Not Equals Date\n",
            "Test Data/Values: thisDate, otherDate\n",
...
{
    Assert( thisDate < otherDate,
            "Test 9: operator < Overloading - Less Than Date\n",
            "Test Data/Values: thisDate, otherDate\n",
            "Expected Output: 'thisDate is Less than otherDate");
}

void TestDateGreaterThanOperator( const Date &thisDate, const Date &otherDate )
{
    Assert( thisDate > otherDate,
            "Test 10: operator > Overloading - Greater Than Date\n",
            "Test Data/Values: thisDate, otherDate\n",
            "Expected Output: 'thisDate is Greater than otherDate");
}
...
void TestOperatingInputStream( Date &date, istringstream &inputStream )
{
    ...
    Assert( date.GetDay() == 10 &&
                    date.GetMonth() == 11 &&
                    date.GetYear() == 2022,
            "Test 12: Operator >> Overloading - Valid input\n",
